...
export const RecommendedProjects: React.FC<{ recommendations: Recommendation[] }> = ({ recommendations }) => {
  return (
    <div className="pt-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {recommendations.length === 0 && (
          <div className="p-5 bg-[#0A0A0A] border border-white/10 rounded-xl text-sm text-neutral-400">
            Add skills to generate recommended project ideas.
          </div>
        )}
        {recommendations.map((rec, index) => (
          <div key={`${rec.title}-${index}`} className="p-5 bg-[#0A0A0A] border border-white/10 rounded-xl hover:border-primary/50 transition-colors cursor-pointer group">
            <div className="flex justify-between items-start mb-3">
              ...
              <div className="text-xs font-mono text-emerald-500">{rec.boostLabel}</div>
            </div>
            <h4 className="font-semibold mb-1 group-hover:text-primary transition-colors">{rec.title}</h4>
            <p className="text-xs text-neutral-500 mb-4 line-clamp-2">{rec.description}</p>
            <div className="flex gap-2 flex-wrap">
              {rec.tags.map((tag) => (
                <span key={tag} className="text-[10px] px-2 py-1 bg-white/5 rounded text-neutral-400">{tag}</span>
              ))}
            </div>
          </div>
        ))}
