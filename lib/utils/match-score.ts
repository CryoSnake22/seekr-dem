...
export function calculateMatchScore(
  ...
  for (const marketSkill of marketSkills) {
    ...
    const userSkillIndex = userSkillsLower.indexOf(
      marketSkill.skill_name.toLowerCase(),
    );
    ...
    if (userSkillIndex !== -1) {
      ...
      const profWeight = userSkill.proficiency_level
        ? PROFICIENCY_WEIGHTS[
            userSkill.proficiency_level as keyof typeof PROFICIENCY_WEIGHTS
          ] || 0.5
...
export function getMissingSkills(
  ...
  const missing = marketSkills
    .filter(
      (skill) => !userSkillsLower.includes(skill.skill_name.toLowerCase()),
    )
    .map((skill) => ({
      name: skill.skill_name,
      frequency: skill.frequency_percentage,
      priority:
        skill.frequency_percentage > 50
          ? ("High" as const)
...
export async function calculateMatchScoreForRole(
  ...
  const userSkillsData: UserSkill[] = (userSkills || []).map((s) => ({
    name: s.skill_name,
    proficiency_level: s.proficiency || undefined,
  }));
